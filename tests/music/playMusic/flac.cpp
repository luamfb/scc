...
// FIXME no sound...
// Tested with 3 different flac files, also tested without SCC
// (no_SCC_flac_test.cpp); the problem must be in SDL_Mixer.
int main(int argc, char **argv)
{
	Uint32 sdlFlags = SDL_INIT_VIDEO | SDL_INIT_AUDIO;
	int mixerFlags = MIX_INIT_FLAC;
	if(!init(sdlFlags, mixerFlags)) {
		SDL_LogCritical(SDL_LOG_CATEGORY_ERROR,
			"couldn't initialize SDL: %s\n", SDL_GetError());
		return ERR_SDL_INIT;
	}
	test(filename, MUS_FLAC);
	quit();
	return 0;
}
